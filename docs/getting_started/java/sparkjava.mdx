...
Getting started with GraphQLize in [Spark Java](http://sparkjava.com/) is simple and involves only few steps.
...
To initialize `GraphQLizeResolver` we need a datasource representing the underlying database connection.
...
With the datasource in place, all we need to do is the create a new instance of `GraphQLizeResolver` using it. 
