...
One of the core design goal of GraphQLize is not to tie to any web development library/framework. It can be used a drop-in library in any Clojure project. GraphQLize built on top of [Lacinia](https://lacinia.readthedocs.io/en/latest/), a de-facto library for implementing GraphQL APIs in Clojure.
...
:::note
To keep things simple, this sample uses `def` to define the states. In a real-word project, you can replace it with [Component](https://github.com/stuartsierra/component), [Mount](https://github.com/tolitius/mount), or [Integrant](https://github.com/weavejester/integrant).
:::
...
With the datasource in place, all we need to do is the create a lacinia schema using it. 
...
With the `lacinia-schema` in place, we can query the undelying database using GraphQL by invoking the Lacinia's [execute](https://walmartlabs.github.io/apidocs/lacinia/com.walmartlabs.lacinia.html#var-execute) function.
...
Congrats! You are on course to build awesome applications using GraphQLize in less time. To save yourself some more time, do refer [this documentation](../../graphql_schema/types-and-fields) to know more about how GraphQLize generates the GraphQL schema and the queries. 
